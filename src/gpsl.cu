...
GPSL::GPSL(CSR& csr_, int device_, int number_of_warps_, BP* bp_ptr_, vector<IDType>* ranks_ptr, vector<IDType>* order_ptr, vector<IDType>* cut_ptr_)
: csr(csr_), device(device_), number_of_warps(number_of_warps_), bp_ptr(bp_ptr_), cut_ptr(cut_ptr_) {
...
void GPSL::Init(){
  ...
  GPSL_InitArrayManager_Kernel<<<1,1>>>(device_array_manager, device_init_buffer);
  cudaDeviceSynchronize();
