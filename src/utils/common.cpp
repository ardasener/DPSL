...
void WriteStats(const vector<Stats> &stats_vec, string filename) {
  ...
  for (int i = 0; i < stats_vec.size(); i++) {
    ...
    ofs << stats.prune_rank << "," << stats.prune_labels << ","
        << stats.prune_global_bp << "," << stats.prune_local_bp << ","
        << stats.total_label_count << "," << stats.avg_label_count << ","
        << stats.time_elapsed << endl;
  }
}
...
void CSR::Reorder(vector<IDType> &order, vector<IDType> *cut,
                  vector<bool> *in_cut) {
